    ...
    <div className="flex justify-between px-6 py-4">
      {days.map((day) => {
        ...
        return (
          ...
          >
            <span className={cn("text-xs font-medium z-10 transition-colors", isSelected ? "text-white" : "text-[#8c8c8c]")}>
              {format(day, 'EEE')}
            </span>
            <div
              className={cn(
                "w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium z-10 transition-colors",
                isSelected ? "text-white" : "bg-white text-[#2d2d2d] shadow-sm"
              )}
            >
              {format(day, 'd')}
            </div>
            {isSelected && (
              <motion.div
                ...
                className="absolute inset-0 bg-[#2d2d2d] rounded-full -top-2 -bottom-2 -left-1 -right-1 z-0"
