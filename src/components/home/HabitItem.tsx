    ...
    >
      ...
      <button
        ...
        className={cn(
          ...
          isCompleted ? "border-[#f27d26] bg-[#f27d26]" : "border-[#d1d1d1] dark:border-[#333] bg-[#f8f6f2] dark:bg-[#1a1a1a]"
        )}
      >
        <motion.div
          initial={false}
      ...
      <motion.div 
        ...
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
        className="flex-1 bg-white dark:bg-[#1a1a1a] rounded-2xl p-4 shadow-sm flex flex-col gap-3 cursor-pointer active:scale-95 transition-transform overflow-hidden relative"
      >
        <div className="flex items-center justify-between">
            ...
            <div>
                <h3 className={cn("text-[#2d2d2d] dark:text-white font-bold text-sm transition-all duration-300 tracking-tight", isCompleted && "line-through text-[#8c8c8c]")}>{habit.name}</h3>
            ...
            <div className="flex items-center gap-3">
                {habit.duration && (
                    <button 
                        ...
                        className="w-8 h-8 rounded-full bg-[#f8f6f2] dark:bg-[#252525] flex items-center justify-center text-[#2d2d2d] dark:text-white hover:bg-orange-50 dark:hover:bg-orange-900/20 hover:text-[#f27d26] transition-colors"
                    >
                        <Plus size={16} />
                    </button>
                )}
                <div className="flex flex-col items-center gap-1 text-[#8c8c8c]">
                    <Clock size={16} />
                    <span className="text-[10px] whitespace-nowrap lowercase">{habit.duration || 1} {habit.unit || 'unid'}</span>
                </div>
            </div>
        </div>
        ...
        {habit.duration && (
            <div className="space-y-1">
                <div className="flex justify-between items-center text-[8px] font-black uppercase tracking-widest">
                    ...
                    <span className="text-[#2d2d2d] dark:text-white">{currentAmount} / {habit.duration} {habit.unit}</span>
                </div>
                <div className="w-full h-1.5 bg-[#f8f6f2] dark:bg-[#252525] rounded-full overflow-hidden">
                    <motion.div 
                        initial={{ width: 0 }}
